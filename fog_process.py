...
# Swapped NBM for NamNest and HREF
MODEL_CONFIGS = [
...
# ================= 1. RTMA ANALYSIS (Crossover Baseline) =================
...
try:
    ...
    print(f"RTMA Error: {e}. Using fallback thresholds.")
    # Fallback grid logic would go here if RTMA fails
...
for cfg in MODEL_CONFIGS:
    ...
    for fxx in range(1, 19):
        try:
            ...
            # --- CONCISE TEXT-ONLY LEGEND (Above the Plot Box) ---
            ax.text(1.0, 1.05, 'Dense Fog (< 1/2 SM)', color='purple', fontsize=12, fontweight='bold', 
                    ha='right', transform=ax.transAxes)
            ax.text(1.0, 1.01, 'Mist (1-3 SM)', color='#E6AC00', fontsize=12, fontweight='bold', 
                    ha='right', transform=ax.transAxes)

            for lon, lat, name in CITIES:
                ax.plot(lon, lat, 'ko', markersize=5, transform=ccrs.PlateCarree())
                ax.text(lon + 0.05, lat + 0.05, name, transform=ccrs.PlateCarree(), fontsize=10, fontweight='bold', 
